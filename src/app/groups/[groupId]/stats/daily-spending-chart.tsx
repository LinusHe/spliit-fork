...
const COLOR_SCALE = [
  'hsl(142, 71%, 45%)', // green — low
  'hsl(80, 60%, 50%)',  // lime
  'hsl(48, 89%, 50%)',  // yellow
  'hsl(36, 90%, 50%)',  // orange
  'hsl(0, 72%, 51%)',   // red — high
]
...
function getBarColor(amount: number, max: number): string {
  ...
  // Interpolate between two adjacent colors for smoother gradient
  const lowerIdx = index
  const upperIdx = Math.min(index + 1, COLOR_SCALE.length - 1)
  const localRatio =
    ratio * (COLOR_SCALE.length - 1) - lowerIdx
  // Simple: just use the closest color
  return localRatio > 0.5 ? COLOR_SCALE[upperIdx] : COLOR_SCALE[lowerIdx]
}
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground px-1">
        <div className="flex items-center gap-1.5">
          <div className="w-8 h-2 rounded-full" style={{ background: `linear-gradient(to right, ${COLOR_SCALE[0]}, ${COLOR_SCALE[2]}, ${COLOR_SCALE[4]})` }} />
