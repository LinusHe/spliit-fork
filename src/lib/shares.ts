...
/**
 * {@link getParticipantShare} for rows with a nested `participant` (the shape
 * Prisma queries with `paidFor: { select: { participant: { select: { id } } } }`
 * return). Returns `null` when the participant is not part of the expense.
 */
export function participantShareOf(
  participantId: string,
  expense: {
    id?: string | null
    amount: number
    splitMode: SplitMode
    paidFor: { participant: { id: string }; shares: number | null }[]
  },
): number | null {
  if (!expense.paidFor.some((p) => p.participant.id === participantId))
    return null
  return getParticipantShare(participantId, {
    id: expense.id,
